$(document).ready(function (){
  ...
  },
  {
    ...
    for (var i= 0; i < twitchApi.length; i++ ) 
        if (twitchApi[i].stream != null)
            {
        {
            $("#results").append("<div class=well>" + "<img src=" + twitchApi[i].logo + "/>" + twitchApi[i].stream.display_name + "</div>");
            $("#dom_element").val(twitchApi[i].stream.display_name);
            console.log(twitchApi[i].stream.display_name);
        }
            }
